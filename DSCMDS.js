...
module.exports = (message, client, args) => {
  if (args[0] == "unregister") {
    if (!gset[message.guild.id]) {
      ...
      return message.channel.send(
        "Use !registerserver to register your server"
      );
    }
  ...
  if (!gset[message.guild.id])
    return message.channel.send(
      "You must register this server first to use this command \n type !server register"
    );

  if (args[0] == "annchannel") {
    if (
      !gset[message.guild.id].ann_channel ||
